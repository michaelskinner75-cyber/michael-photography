-- Idempotent repair migration for Michael Skinner Photography.
...
alter table public.albums add column if not exists expires_at date;
...
create policy "Public can view public albums" on public.albums
for select using (
  auth.role() = 'authenticated'
  or (is_public = true and (expires_at is null or expires_at >= current_date))
);

drop policy if exists "Public can view photos in public albums" on public.photos;
create policy "Public can view photos in public albums" on public.photos
for select using (
  exists (
    ...
    where albums.id = photos.album_id
      and (
        auth.role() = 'authenticated'
        or (albums.is_public = true and (albums.expires_at is null or albums.expires_at >= current_date))
      )
  )
);

drop policy if exists "Authenticated users manage albums" on public.albums;
create policy "Authenticated users manage albums" on public.albums
for all to authenticated using (true) with check (true);

drop policy if exists "Authenticated users manage photos" on public.photos;
create policy "Authenticated users manage photos" on public.photos
for all to authenticated using (true) with check (true);
...
create policy "Public photo viewing" on storage.objects
...
create policy "Authenticated photo update" on storage.objects
...
create policy "Authenticated photo deletion" on storage.objects
