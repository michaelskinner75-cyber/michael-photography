...
async function loadAlbums(){
  ...
  const{data,error}=await client.from('albums').select('id,title,description,event_date,expires_at,cover_url,created_at,view_count').eq('is_public',true).or(`expires_at.is.null,expires_at.gt.${now}`).order('event_date',{ascending:false,nullsFirst:false});
  if(error){albumGrid.innerHTML='<p>Albums could not be loaded.</p>';return}
  albumGrid.innerHTML=data?.length?data.map(a=>`<button class="album-card" data-id="${a.id}"><div class="album-cover" style="background-image:url('${a.cover_url||''}')"><span>${a.cover_url?'':'MS'}</span></div><div class="album-meta"><small>${escapeHtml(formatDate(a.event_date))}</small><h3>${escapeHtml(a.title)}</h3><span>View album →</span><div class="album-stats">👁 ${Number(a.view_count||0).toLocaleString('en-GB')} views</div></div></button>`).join(''):'<p>No public albums have been added yet.</p>';
  albumGrid.querySelectorAll('.album-card').forEach(c=>c.addEventListener('click',()=>openAlbum(c.dataset.id)));
}
...
async function loadReactionCounts(photoIds){
  if(!photoIds.length)return;
  const{data}=await client.from('photo_reactions').select('photo_id,visitor_id,reaction').in('photo_id',photoIds);
  photoIds.forEach(id=>reactionCounts[id]={like:0,love:0,mine:null});
  (data||[]).forEach(r=>{const c=reactionCounts[r.photo_id]||(reactionCounts[r.photo_id]={like:0,love:0,mine:null});c[r.reaction]+=1;if(r.visitor_id===visitorId)c.mine=r.reaction});
}
...
async function openAlbum(id){
  ...
  const card=albumGrid.querySelector(`.album-card[data-id="${id}"] .album-stats`);if(card)card.textContent=`👁 ${Number(newCount||0).toLocaleString('en-GB')} views`;
  await loadReactionCounts((photos||[]).map(p=>p.id));
  ...
  const images=[...grid.querySelectorAll('img')];let loaded=0;const done=()=>updatePhotoProgress(++loaded,images.length);if(!images.length)updatePhotoProgress(0,0);images.forEach(i=>i.complete?done():(i.addEventListener('load',done,{once:true}),i.addEventListener('error',done,{once:true})));
